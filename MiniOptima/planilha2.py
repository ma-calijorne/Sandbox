...
def miniOptima():
	# wb = xw.Book(r'C:/Users/Usuario/Documents/Mini Optma/planilha2.xlsx')
	wb = xw.Book.caller()

	Sheet1 = wb.sheets[0]

	meta_lucro = float(Sheet1.range('G15').value)
	meta_custo = float(Sheet1.range('I16').value)
	...
	try:
		...
		jj = 0
		for i in row:
			Sheet1.range('G{}'.format(ii)).value = i
			total_lucros += i *c[jj]*l[jj]
			ii += 1
			jj += 1

		Sheet1.range('G15').value = total_lucros
		# print(row)
	except CplexError as exc:
		Sheet1.range('K1').value = 'Não foi encontrada solução viável'
	...
	try:
		...
		jj = 0
		for i in row:
			Sheet1.range('I{}'.format(ii)).value = i
			total_custos += i *c[jj]
			ii += 1
			jj += 1
		Sheet1.range('I16').value = total_custos
		Sheet1.range('K2').value = ''
		# print(row)
	except CplexError as exc:
		Sheet1.range('K2').value = 'Não foi encontrada solução viável'
